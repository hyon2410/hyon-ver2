...
#define GYRO_RATE            16.4f     // ±2000 dps
#define ACCELERATION_RATE    4096.0f   // ±8g
#define ACCELERATION_RATE_Z  4096.0f
...
float gyro_filter_coeff=0.0; //0.7
...
uint8_t MPU6050_Init_Benim(I2C_HandleTypeDef *I2Cx) {
    ...
    if (check == 104)  // 0x68 will be returned by the sensor if everything goes well
    {
        ...
        Data = 0x00;   // 1kHz (gyro output rate / (1 + div))
        ...
        // XG_ST=0,YG_ST=0,ZG_ST=0, FS_SEL=0 -> ï¿½ 250 ï¿½/s 0x01
        Data = (3 << 3);   // FS_SEL = 3 → ±2000 dps
...
void calibrate_mpu6050(){
    printf("Dang calibrate YAW (Gyro Z)...\r\n");
    while(!calibrate_yaw(&hi2c2, &MPU6050_1));
    printf("Calibrate YAW xong.\r\n");

    printf("Dang calibrate ACC offsets...\r\n");
    while(!calibrate_Benim(&hi2c2, &MPU6050_1));
    printf("Calibrate ACC xong.\r\n");

    fill_calibration_buffer();
    // ... sau fill_calibration_buffer()
    printf("Luu MPU calib vao flash...\r\n");
    HAL_FLASH_Unlock();
    FLASH_Erase_Sector(FLASH_SECTOR_11, FLASH_VOLTAGE_RANGE_3);
    if(flash_write_calib_float_safe(Sector15_Address, calibration_buffer_float, 6) == HAL_OK) {
        printf("Luu MPU calib THANH CONG!\r\n");
    } else {
        printf("Luu MPU calib THAT BAI!\r\n");
    }
    HAL_FLASH_Lock();
    printf("Gia tri calib:\r\n");
    printf("pitch_cal = %.2f   roll_cal = %.2f   yaw_cal = %.2f\r\n",
           MPU6050_1.pitch_calibration_value,
           MPU6050_1.roll_calibration_value,
           MPU6050_1.yaw_calibration_value);
    printf("Xoff = %.2f   Yoff = %.2f   Zoff = %.2f\r\n",
           MPU6050_1.Accel_X_Raw_Offset,
           MPU6050_1.Accel_Y_Raw_Offset,
           MPU6050_1.Accel_Z_Raw_Rate);

    printf("Dang luu vao flash...\r\n");
    flash_write_calib_data_four_byte((uint32_t*)calibration_buffer_float, 6); // Ép kiểu an toàn
    printf("Luu flash THANH CONG!\r\n");
}
